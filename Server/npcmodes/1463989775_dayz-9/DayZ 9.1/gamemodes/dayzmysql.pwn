...
#define mysql_database  "dayzsa"
//achievements
#define function%0(%1) forward%0(%1);public%0(%1)
#define Web "www.pgagaming.net"
#define Country "[UNITED STATES]"
#define Version "v9.1"
#define Creator "Kitten aka AldenJ"
#define Name "GTA SA: DayZ Version "Country" ["Version"]"
...
#define MAX_ZOMBIE_FOR_PLAYER 	(2)
...
stock Float:DayZ_Z_absoluteangle(Float:angle)
{
	while(angle < 0.0)angle += 360.0;
	while(angle > 360.0)angle -= 360.0;
	return angle;
}
stock Float:DayZ_Z_GetAngleToPoint(Float:fPointX, Float:fPointY, Float:fDestX, Float:fDestY)//Thank Southclaw
	return DayZ_Z_absoluteangle(-(90-(atan2((fDestY - fPointY), (fDestX - fPointX)))));
...
new randomMessages[][] =
{
	"*"COL_LIGHTBLUE" Buy Premium/V.I.P at www.website.com",
 	"*"COL_LIGHTBLUE" Join our forums today! "Web"",
  	"*"COL_LIGHTBLUE" Purchasing Premium/V.I.P helps us pay for our service",
  	"*"COL_LIGHTBLUE" Use global chat to talk to others via /o or /ooc",
  	"*"COL_LIGHTBLUE" Download modpack 4.0 via www.dayzsa.net/download/modpack4.0.exe",
  	"*"COL_LIGHTBLUE" Remember to report hackers and rule breakers at /report [id] [reason]",
  	"*"COL_LIGHTBLUE" Join our teamspeak 3 to communicate easier with friends! IP",
...
public OnPlayerEnterCheckpoint(playerid)
{
	if(IsPlayerInAnyVehicle(playerid))
 	return SendClientMessage(playerid, -1,"{ff0000}You can't get airdrop from vehicle!");
    ...
    format(string, sizeof string,"{ff0000}[AIRDROP]{ffffff}: %s  took the Airdrop",nome);
    ...
    ShowPlayerDialog(playerid, DIALOG_AIRDROP, DIALOG_STYLE_LIST,"AirDrop","{ffffff}Pizza Slice\n{ffffff}Rum\n{ffffff}Full Water Bottle\n{0000FF}Apple\n{0000FF}Rope\n{fff700}Flashlight\n{ff0000}M4\n{ff0000}Assault Ammo\n{ff00b3}Pistol\n{ff00b3}Pistol Ammo\n{00ff00}Sniper Rifle\n{00ff00}Sniper Ammo","Take","Close");
    return 1;
}
forward Airdrop();
public Airdrop()
{
    ...
    for(new i=0; i < MAX_PLAYERS; i++)
    {
        ...
        SetPlayerCheckpoint(i, 306.42618, 2026.72473, 16.73662, 3);
    }

    //Enviar Mensagem Que o Airdrop Foi Solto
    SendClientMessageToAll(-1, "{ff0000}[AIRDROP]: {ffffff}Military Supply Drop has been dropped at Area 51.");
    printf(" ");
		...
		case DIALOG_AIRDROP:
  		{
	        if(response)
	        {
	            switch(listitem)
	            {
	                ...
	                case 0:  AddSlotToInventoryWorld(playerid,"Pizza Slice",1);
	                case 1:  AddSlotToInventoryWorld(playerid,"Rum",1);
	                case 2:  AddSlotToInventoryWorld(playerid,"Full Water Bottle",1);
	                case 3:  AddSlotToInventoryWorld(playerid,"Apple",1);
	                case 4:  AddSlotToInventoryWorld(playerid,"Rope",1);
	                case 5:  AddSlotToInventoryWorld(playerid,"Flashlight",1);
	                case 6:  AddSlotToInventoryWorld(playerid,"M4",1);
	                case 7:  AddSlotToInventoryWorld(playerid,"Assault Ammo",1);
	                case 8:  AddSlotToInventoryWorld(playerid,"Pistol",1);
	                case 9:  AddSlotToInventoryWorld(playerid,"Pistol Ammo",1);
	                case 10: AddSlotToInventoryWorld(playerid,"Sniper Rifle",1);
	                case 11: AddSlotToInventoryWorld(playerid,"Sniper Ammo",1);
	            }
	        }
    	}
...
CMD:tietest(playerid,params[])
{
   	if(PlayerHasItem(playerid, "Rope"))
    {
		new id;
        TogglePlayerControllable(playerid,0);
        pInfo[id][pKidnapped] = 1;
    }
    return 1;
}
CMD:untie(playerid,params[])
{
    ...
    {
        new id;
  		if(sscanf(params, "u", id)) SendClientMessage(playerid, COLOR_RED, "USAGE: /unfreeze [playerid]"); //tell sscanf again if the parameters/syntax is wrong to return a special message
        if(!IsPlayerConnected(id)) SendClientMessage(playerid, COLOR_RED,  "ERROR: Invalid playerid");
        TogglePlayerControllable(id,1);
        SendClientMessage(playerid,COLOR_RED," You are untied ");
        pInfo[id][pKidnapped] = 0;
    }
    return 1;
}
CMD:tie(playerid,params[])
{
    if(PlayerHasItem(playerid, "Rope"))
    {
        ...
        SendClientMessage(playerid, COLOR_RED, "You are Kidnapped ! ");
        pInfo[id][pKidnapped] = 1;
    }
    return 1;
}
...
{
new string[128];
format(string, sizeof(string),""COL_RED"[Admin] %s",params );
SendClientMessageToAll(-1, string);
}
...
// USED FOR DEBUGING PROCESS
CMD:orange(playerid)
{
        if(pInfo[playerid][pAdminLevel] >= 5)
        {
            AddSlotToInventoryWorld(playerid,"Lighter",1);
            AddSlotToInventoryWorld(playerid,"Wood",1);
            AddSlotToInventoryWorld(playerid,"Dirty Water Bottle",1);
            AddSlotToInventoryWorld(playerid,"Rope",1);

        }
                else {
                SendClientMessage(playerid,-1,""COL_WHITE"SERVER: Unknown command.");
        }
        return 1;
}
...
{
    new string[128];
    format(string, sizeof(string), "%s Says: I am friendly.", PlayerName(playerid), params);
    ProxDetector(30.0, playerid, string, COLOR_GREY);
    return 1;
    }
CMD:t2(playerid, params[])
{
    new string[128];
    format(string, sizeof(string), "%s Says: Drop your weapons", PlayerName(playerid), params);
    ProxDetector(30.0, playerid, string, COLOR_GREY);
    return 1;
    }
...
CMD:stuck(playerid,params[])
{
		...
		SetPlayerPos_Allow(playerid, posxx[0], posxx[1], posxx[2]+2);
		SendClientMessage(playerid,-1,"*"COL_RED" Slapped!");
		return 1;
}
...
stock CreatePlayerTD(playerid)
{
    if(!IsPlayerNPC(playerid))
	{
		WebsiteLogo[playerid] = CreatePlayerTextDraw(playerid,501.000000, 105.000000, "www.pgagaming.net");
		PlayerTextDrawBackgroundColor(playerid,WebsiteLogo[playerid], 255);
		PlayerTextDrawFont(playerid,WebsiteLogo[playerid], 2);
		PlayerTextDrawLetterSize(playerid,WebsiteLogo[playerid], 0.250000, 1.200000);
		PlayerTextDrawColor(playerid,WebsiteLogo[playerid], -1);
		PlayerTextDrawSetOutline(playerid,WebsiteLogo[playerid], 0);
		PlayerTextDrawSetProportional(playerid,WebsiteLogo[playerid], 1);
		PlayerTextDrawSetShadow(playerid,WebsiteLogo[playerid], 1);
		...
		MenuCredits[playerid] = CreatePlayerTextDraw(playerid,320.000000, 269.000000, "~w~Created by~g~ [pGA]AldenJ~w~~n~www.pgagaming.net");
		...
		DebugMonitor_Text[playerid] = CreatePlayerTextDraw(playerid,520.000000, 11.000000, "DEBUG MONITOR:");
		PlayerTextDrawBackgroundColor(playerid,DebugMonitor_Text[playerid], 255);
		PlayerTextDrawFont(playerid,DebugMonitor_Text[playerid], 1);
		PlayerTextDrawLetterSize(playerid,DebugMonitor_Text[playerid], 0.270000, 1.100000);
		PlayerTextDrawColor(playerid,DebugMonitor_Text[playerid], -1);
		PlayerTextDrawSetOutline(playerid,DebugMonitor_Text[playerid], 0);
		PlayerTextDrawSetProportional(playerid,DebugMonitor_Text[playerid], 1);
		PlayerTextDrawSetShadow(playerid,DebugMonitor_Text[playerid], 1);
		PlayerTextDrawSetSelectable(playerid,DebugMonitor_Text[playerid], 0);

		DebugInfo[playerid] = CreatePlayerTextDraw(playerid,556.000000, 29.000000, "Zombies Killed: 99~n~Headshots: 99~n~Murders: 99~n~Bandits Killed: 99~n~Zombies: 0~n~Temperature: 42 C~n~Name: [pGA]AldenJ");
		...
		PlayerTextDrawLetterSize(playerid,DebugInfo[playerid], 0.239999, 0.799998);
		...
		Debug_GunIcon[playerid] = CreatePlayerTextDraw(playerid,500.000000, 87.000000, "hud:radar_emmetgun");
		PlayerTextDrawBackgroundColor(playerid,Debug_GunIcon[playerid], 255);
		PlayerTextDrawFont(playerid,Debug_GunIcon[playerid], 4);
		PlayerTextDrawLetterSize(playerid,Debug_GunIcon[playerid], 0.579999, 1.899999);
		PlayerTextDrawColor(playerid,Debug_GunIcon[playerid], -16776961);
		...
		PlayerTextDrawBoxColor(playerid,Debug_GunIcon[playerid], 5243060);
		PlayerTextDrawTextSize(playerid,Debug_GunIcon[playerid], 13.000000, 11.000000);
		PlayerTextDrawSetSelectable(playerid,Debug_GunIcon[playerid], 0);

		Debug_GunInfo[playerid] = CreatePlayerTextDraw(playerid,530.000000, 92.000000, "Sniper Rifle / 10 Ammo");
		PlayerTextDrawBackgroundColor(playerid,Debug_GunInfo[playerid], 255);
		PlayerTextDrawFont(playerid,Debug_GunInfo[playerid], 1);
		PlayerTextDrawLetterSize(playerid,Debug_GunInfo[playerid], 0.209999, 0.799998);
		PlayerTextDrawColor(playerid,Debug_GunInfo[playerid], -1);
		PlayerTextDrawSetOutline(playerid,Debug_GunInfo[playerid], 0);
		PlayerTextDrawSetProportional(playerid,Debug_GunInfo[playerid], 1);
		PlayerTextDrawSetShadow(playerid,Debug_GunInfo[playerid], 1);
...
stock CreateTDS()
{
	...
	DebugBox = TextDrawCreate(494.000000, 8.000000, "ld_dual:backgnd");
	TextDrawBackgroundColor(DebugBox, 255);
	TextDrawFont(DebugBox, 4);
	TextDrawLetterSize(DebugBox, 1.500000, 1.000000);
	TextDrawColor(DebugBox, -1);
	TextDrawSetOutline(DebugBox, 0);
	TextDrawSetProportional(DebugBox, 1);
	TextDrawSetShadow(DebugBox, 1);
	TextDrawUseBox(DebugBox, 1);
	TextDrawBoxColor(DebugBox, 255);
	TextDrawTextSize(DebugBox, 124.000000, 97.000000);
	TextDrawSetSelectable(DebugBox, 0);

	DebugText = TextDrawCreate(504.000000, 14.000000, "DEBUG MONITOR");
	TextDrawBackgroundColor(DebugText, 255);
	TextDrawFont(DebugText, 1);
	TextDrawLetterSize(DebugText, 0.400000, 1.000000);
	TextDrawColor(DebugText, -1);
	TextDrawSetOutline(DebugText, 0);
	TextDrawSetProportional(DebugText, 1);
	TextDrawSetShadow(DebugText, 1);
	TextDrawSetSelectable(DebugText, 0);

	DayZSA_Web = TextDrawCreate(501.000000, 127.000000, "www.pgagaming.net");
	TextDrawBackgroundColor(DayZSA_Web, 255);
	TextDrawFont(DayZSA_Web, 2);
	TextDrawLetterSize(DayZSA_Web, 0.289999, 1.000000);
	TextDrawColor(DayZSA_Web, -1);
	TextDrawSetOutline(DayZSA_Web, 0);
	TextDrawSetProportional(DayZSA_Web, 1);
	TextDrawSetShadow(DayZSA_Web, 1);
...
public OnPlayerUseItem(playerid,ItemName[])
{
    if(!IsPlayerNPC(playerid))
    {
		...
		if(!strcmp(ItemName,"Pizza Slice",true))
	 	{
	 	    if(pInfo[playerid][pHunger] >= 2350) return SendClientMessage(playerid,-1,""chat" You are not hungry!");
			pInfo[playerid][pHunger] += 950;
			pInfo[playerid][pBlood] += random(350);
			RemoveSlotToInventory(playerid,"Pizza Slice",1);
  			if(!IsPlayerInAnyVehicle(playerid))
	    	{
				ApplyAnimation(playerid, "FOOD", "EAT_Burger",4.1,0,0,0,1000,1);
				defer ResetAction(playerid, ACTION_ANIMATION);
...
function DayZSA_LootSetupMarket()
{
	...
	switch(random(14))
	{
	    ...
	    case 4: format(str,sizeof(str),"Pizza Slice"),dayzmodel = 2814, rotx = 0.0, roty = 0.0, rotz = 0.0;
...
function DayZSA_LootSetupLow()
{
	...
	switch(random(25))
	{
	    ...
	    case 2: format(str,sizeof(str),"Pizza Slice"),dayzmodel = 2814, rotx = 0.0, roty = 0.0, rotz = 0.0;
		...
		case 13: format(str,sizeof(str),"Pizza Slice"),dayzmodel = 2814, rotx = 0.0, roty = 0.0, rotz = 0.0;
...
function DayZSA_LootSetupMedium()
{
	...
	switch(random(35))
	{
		...
		case 18: format(str,sizeof(str),"Pizza Slice"),dayzmodel = 2814, rotx = 0.0, roty = 0.0, rotz = 0.0;
...
function DayZSA_CreateVehicleStorage()
{
    ...
    for(new i = 0; i < MAX_VEHICLES; i++)
    {
		...
		switch(random(4))
		{
			case 0: SetVehicleHealth(i,480.0);
			case 1: SetVehicleHealth(i,800.0);
			case 2: SetVehicleHealth(i,580.0);
			case 3: SetVehicleHealth(i,400.0);
		}
...
function DayZSA_CreateLootsDeadBody(playerid,Float:pXX,Float:pYX,Float:pZX)
{
	...
	if(PlayerHasItem(playerid, "Pizza Slice")) DayZSA_CreateLootsFunction("Pizza Slice",pXX+random(2),pYX+random(4),pZX);
...
function DayZSA_LootModelDrop(objectname[])
{
	...
	if(strfind(objectname, "Pizza Slice", true) != -1) dayzsaid = 2814, rotx = 0.0, roty = 0.0, rotz = 0.0;
...
stock FCNPC_GoToPlayerEx(npcid, playerid, Float:dist, Float:rangle, movetype = MOVE_TYPE_AUTO)
{
    ...
    FCNPC_GoTo(npcid,x,y,z,movetype,1,true);
    return 1;
}
